"""The `gapit db fetch` command: checksum-verified local-file installation.

Local filesystem ONLY: SOURCE must be a path to an existing regular file.
There is no network fetch, no provider IDs, no archives, no manifests — this
is the generic installation slice (streaming SHA256 + atomic copy), kept
deliberately independent of db.py and the screening pipeline.
"""
...
def install_verified(source: Path, target: Path, expected: str) -> FetchReceipt:
    ...
    try:
        with source.open("rb") as src, NamedTemporaryFile(
...
def db_fetch_command(
    ...
    LOCAL FILESYSTEM ONLY: this never fetches over the network and knows
    nothing about database providers or sequence content — it checksum-verifies
    and atomically installs bytes. On success a one-line JSON receipt
    (destination, verified digest) is printed to stdout.
    """
...
def register_db_command(app: typer.Typer) -> None:
    """Attach the `db` command group (fetch) to the CLI app."""
    db_app = typer.Typer(
        help="Database file maintenance (local filesystem only; no network).",
